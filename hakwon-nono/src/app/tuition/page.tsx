...
export default function TuitionPage() {
  ...
  const realmAvgData = useMemo(() => {
    ...
    return Object.entries(realmMap)
      .map(([realm, v]) => ({
        realm,
        fullName: realm,
        label: REALM_LABELS[realm] || realm,
        avgTuition: Math.round(v.totalTuition / v.totalCount),
        count: v.totalCount,
        color: REALM_COLORS[realm] || '#6B7280',
      }))
      .sort((a, b) => b.avgTuition - a.avgTuition);
  }, [data]);
  ...
  const getHeatColor = (value: number | undefined, maxVal: number) => {
    if (!value || !maxVal) return 'bg-gray-50';
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white border-b border-gray-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 h-14 flex items-center gap-4">
          ...
          <h1 className="text-sm font-medium text-gray-600">사교육비 물가 지수</h1>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        {/* 페이지 타이틀 */}
        <div className="mb-8">
          <h2 className="text-2xl font-bold text-gray-900">사교육비 물가 지수</h2>
        ...
        {isLoading && (
          <div className="flex items-center justify-center py-32">
            <div className="flex flex-col items-center gap-3">
              ...
              <p className="text-sm text-gray-500">수강료 데이터 분석 중...</p>
            </div>
          </div>
        )}
        ...
        {data && !isLoading && (
          <div className="space-y-8 animate-fade-in-up">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <p className="text-sm text-gray-500">가장 비싼 지역</p>
                </div>
                <p className="text-2xl font-bold text-gray-900">{mostExpensive?.region || '-'}</p>
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <p className="text-sm text-gray-500">가장 저렴한 지역</p>
                </div>
                <p className="text-2xl font-bold text-gray-900">{cheapest?.region || '-'}</p>
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <p className="text-sm text-gray-500">수강료 데이터 건수</p>
                </div>
                <p className="text-3xl font-bold text-gray-900">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
              <h3 className="text-lg font-bold text-gray-800 mb-2">지역별 평균 수강료</h3>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
              <h3 className="text-lg font-bold text-gray-800 mb-4">분야별 평균 수강료 비교</h3>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
              <h3 className="text-lg font-bold text-gray-800 mb-2">지역 x 분야 히트맵</h3>
              <p className="text-sm text-gray-500 mb-4">
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-gray-200">
                      <th className="text-left py-3 px-3 text-xs font-semibold text-gray-500 sticky left-0 bg-white z-10 min-w-[100px]">
                        지역
                      </th>
                      {heatmapData.realms.map((realm) => (
                        <th
                          key={realm}
                          className="text-center py-3 px-2 text-xs font-semibold text-gray-500 min-w-[80px]"
                        >
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {heatmapData.regions.map((region) => (
                      <tr key={region} className="hover:bg-gray-50/50">
                        <td className="py-2.5 px-3 font-medium text-gray-700 sticky left-0 bg-white z-10">
                          {region}
                        </td>
                        {heatmapData.realms.map((realm) => {
